...
class TRGoals:
    ...
    def m3u_guncelle(self):
        ...
        yayin_ara = re.search(r'var baseurl = "(https?:\/\/[^"]+)"', response.text)
        if not yayin_ara:
            iframe_ara = re.search(r'<iframe[^>]+src="(https?:\/\/[^"]+\.m3u8[^"]*)"', response.text)
            if iframe_ara:
                yayin_ara = [None, iframe_ara[1]]
        ...
        if not yayin_ara:
            ...
            if baslik == "404 Not Found":
                ...
                raise ValueError("Base URL veya iframe bulunamadı!")
